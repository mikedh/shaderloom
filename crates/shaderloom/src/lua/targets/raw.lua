...
function raw_target.emit_loose_shaders(options, shaders, env)
    ...
    for _, shader in ipairs(shaders) do
        ...
        if written_names[outfile] then
            ...
            -- (including any header template) and normalize whitespace, and
            -- shorten identifiers to opaque names. Applied to the final templated
            -- text so the autogenerated header is stripped too.
            if options.minify then
                source = loom:minify_wgsl(source)
            end
...
-- The `minify` branch of emit_loose_shaders, end to end: capture fileio.write
-- and assert the written text was actually minified (and renamed).
function tests.minify_loose_shaders_branch()
    ...
    local ok, err = pcall(raw_target.emit_loose_shaders,
        {output = "loose_min.wgsl", minify = true},
...
function tests.minify_binding_direct()
    ...
    assert(not pcall(function() return loom:minify_wgsl("not wgsl {{{") end),
